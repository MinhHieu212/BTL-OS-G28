...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct** frm_lst)
{
  ...
  for(pgit = 0; pgit < req_pgnum; pgit++)
  {
    if(MEMPHY_get_freefp(caller->mram, &fpn) == 0)
   {
    ...
    // BEGIN MY CODE
     int victim_pgn, victim_fpn, swp_free_fpn; 
     uint32_t  victim_pte;
     ...
     if(MEMPHY_get_freefp(caller->active_mswp , &swp_free_fpn) != 0) return -3000;

     __swap_cp_page(caller->mram , victim_fpn , caller->active_mswp , swp_free_fpn);
      
     pte_set_swap(&(caller->mm->pgd[victim_pgn]), 0, swp_free_fpn);
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  mm->pgd = malloc(PAGING_MAX_PGN*sizeof(uint32_t));  // page table
